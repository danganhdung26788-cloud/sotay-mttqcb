...
if (!config.includes("VERSION: '1.1.0'")) {
  console.error('VERSION_MISMATCH'); failed = true;
}

try {
  const manifest = JSON.parse(fs.readFileSync(path.join(src,'appsscript.json'),'utf8'));
  if (manifest.timeZone !== 'Asia/Ho_Chi_Minh') {
    console.error('MANIFEST_TIMEZONE_MISMATCH'); failed = true;
  }
  if (manifest.runtimeVersion !== 'V8') {
    console.error('MANIFEST_RUNTIME_MISMATCH'); failed = true;
  }
} catch (e) {
  console.error('MANIFEST_INVALID_JSON', e.message); failed = true;
}

const claspIgnorePath = path.resolve('.claspignore');
if (!fs.existsSync(claspIgnorePath)) {
  console.error('MISSING .claspignore'); failed = true;
} else {
  const rules = fs.readFileSync(claspIgnorePath,'utf8')
    .split(/\r?\n/).map(x=>x.trim()).filter(Boolean);
  for (const rule of ['!appsscript.json','!**/*.gs','!**/*.html']) {
    if (!rules.includes(rule)) {
      console.error('CLASPIGNORE_MISSING_RUNTIME_RULE', rule); failed = true;
    }
  }
  if (rules.includes('!src/**')) {
    console.error('CLASPIGNORE_WRONG_ROOTDIR_RULE !src/**'); failed = true;
  }
}
...
try {
  ...
  if (pkg.devDependencies?.['@google/clasp'] !== '3.4.1') {
    console.error('CLASP_VERSION_MISMATCH'); failed = true;
  }
} catch (e) {
  console.error('PACKAGE_INVALID_JSON', e.message); failed = true;
}

if (!fs.existsSync(path.resolve('scripts/setup-google-auth.ps1'))) {
  console.error('MISSING_GOOGLE_AUTH_HELPER'); failed = true;
}
